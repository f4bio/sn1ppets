...
writeMessage "find $DIR -type f -exec chmod o+rw,g+r,o+r "{}" \; >> /tmp/fix_permissions.log"
...
writeMessage "find $DIR -type d -exec chmod 755 "{}" \; >> /tmp/fix_permissions.log"
find $DIR -type d -exec chmod 755 "{}" \; >> /tmp/fix_permissions.log
info "Directories-755 Done... (2/4)"

writeMessage "chown -R $UID:$GID $(pwd) >> /tmp/fix_permissions.log"
